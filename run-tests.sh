...
# run_test_with_project_dir "desc" "cmd" "expected" '{"key":...}'
# Like run_test_with_config but simulates CWD drift: the hook's cwd field is
# set to /tmp while CLAUDE_PROJECT_DIR points to the real project dir containing
# the config. Verifies that the engine uses CLAUDE_PROJECT_DIR, not cwd.
run_test_with_project_dir() {
  local description="$1" command="$2" expected="$3" config_json="$4"
  local seq=$(( ++_SEQ ))
  (( TOTAL++ ))
  ...
  local result_file="$_TMPDIR/$seq"
  (
    local proj_dir json output actual
    proj_dir=$(mktemp -d)
    mkdir -p "$proj_dir/.claude"
    printf '%s' "$config_json" > "$proj_dir/.claude/unfence.json"

    # Engine sees cwd=/tmp (drifted), but CLAUDE_PROJECT_DIR points to config
    json=$(jq -n --arg cmd "$command" --arg cwd "/tmp" \
      '{"tool_input":{"command":$cmd},"cwd":$cwd}')
    output=$(CLAUDE_PROJECT_DIR="$proj_dir" bash "$ENGINE" <<< "$json" 2>/dev/null)
    rm -rf "$proj_dir"
...
export -f run_test_with_project_dir
