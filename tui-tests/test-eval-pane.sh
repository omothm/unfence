...
run() {
    ...
    #    Type "git status" — a read-only command that should get a verdict
    for ch in g i t " " s t a t u s; do
        tui_send "$ch" ""
    done
    sleep 0.3
    tui_send Enter ""; sleep 3   # engine subprocess; give it time
